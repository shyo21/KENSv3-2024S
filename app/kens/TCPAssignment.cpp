...
void TCPAssignment::packetArrived(std::string fromModule, Packet &&packet) {
  ...
  if (isSYN && !isACK) {
    ...
    /* TODO: 이미 syn을 받은 적이 있는지 확인 syn 패킷을 받은놈(dest ip dest
    ...
    handShakingMap[destAddrPair] =
        std::make_tuple(srcAddrPair, SocketState::SYN_RCV, -1, seqNum + 1);
  ...
  if (isSYN && isACK) {
    ...
    if (myIter != handShakingMap.end()) {
      if ((std::get<3>(myIter->second) == ackNum) &&
...
TCPAssignment::getSocket(std::pair<uint32_t, in_port_t> addrPair) {
  ...
  for (const auto &setIter : socketSet) {
    if ((setIter->myAddr->sin_addr.s_addr == std::get<0>(addrPair)) &&
        (setIter->myAddr->sin_port == std::get<1>(addrPair))) {
      mySocket = setIter;
    }
  }
  return mySocket;
...
void TCPAssignment::syscall_listen(UUID syscallUUID, int pid, int fd,
  ...
  if (mySocket->socketState != SocketState::BOUND) {
    printf("socketstate not bound in listen");
    this->returnSystemCall(syscallUUID, -1);
    return;
  }
  // socketstate listening으로
...
void TCPAssignment::syscall_connect(UUID syscallUUID, int pid, int fd,
  ...
  // 만약 소켓맵에서 못 찾는다면 에러
  if (mySocket == nullptr) {
    printf("no matching socket in connect");
    this->returnSystemCall(syscallUUID, -1);
    return;
  }
  ...
  handShakingMap[myPair] =
      std::make_tuple(peerPair, SocketState::SYN_SENT, -1, seqNum + 1);
